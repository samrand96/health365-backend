...
@router.get("/health", dependencies=[Depends(has_permission(["admin"]))])
async def health_check():
    return {'Health': DB_URL}


@router.post('/users', dependencies=[Depends(has_permission(["admin"]))])
...
@router.get('/users', dependencies=[Depends(has_permission(["admin"]))], response_model=List[User_Pydantic])
async def get_users():
...
@router.get('/users/{user_id}', dependencies=[Depends(has_permission(["admin"]))], response_model=User_Pydantic)
async def get_user(user_id: int):
    return await User_Pydantic.from_queryset_single(User.get(id=user_id))


@router.put('/users/{user_id}', dependencies=[Depends(has_permission(["admin"]))], response_model=User_Pydantic)
...
@router.delete('/users/{user_id}', dependencies=[Depends(has_permission(["admin"]))])
