...
@router.get("/patients", dependencies=[Depends(has_permission(["admin","doctor"]))], response_model=List[Patient_Pydantic])
async def get_patients(user=Depends(has_permission(["admin", "doctor"]))):
    if user['is_admin']:
        return await Patient_Pydantic.from_queryset(Patient.all())
    else:
        return await Patient_Pydantic.from_queryset(Patient.filter(doctor_id=user['id']))


@router.get("/patients/{patient_id}", dependencies=[Depends(has_permission(["admin","doctor"]))], response_model=Patient_Pydantic)
async def get_patients(patient_id: int, user=Depends(has_permission(["admin", "doctor"]))):
    try:
        if user['is_admin']:
            return await Patient_Pydantic.from_queryset_single(Patient.get(id=patient_id))
        else:
            return await Patient_Pydantic.from_queryset_single(Patient.filter(Q(doctor_id=user['id']) & Q(id=patient_id)).first())
    except Exception as e:
        raise HTTPException(status_code=404, detail="Patient not found")


@router.post("/patients", dependencies=[Depends(has_permission(["admin","doctor"]))], response_model=Patient_Pydantic)
async def create_patient(patient: Patient_Pydantic, user=Depends(has_permission(["admin", "doctor"]))):
    if user['is_admin']:
...
@router.delete("/patients/{patient_id}", dependencies=[Depends(has_permission(["admin","doctor"]))])
...
@router.put("/patients/{patient_id}", dependencies=[Depends(has_permission(["admin","doctor"]))], response_model=Patient_Pydantic)
